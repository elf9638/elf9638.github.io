...
(function (root, factory) {
  ...
  var DateRangePicker = function (element, options, cb) {
    ...
    this.parentEl =
      options.parentEl && $(options.parentEl).length
        ? $(options.parentEl)
        : $(this.parentEl);
    this.container = $(options.template).insertAfter(this.element);
  ...
  DateRangePicker.prototype = {
      ...
      for (var row = 0; row < 6; row++) {
        ...
        for (var col = 0; col < 7; col++) {
          ...
          html +=
            ...
            "</a>";
          if (1) {
            html += "<span class='price'>" + "10.5만원" + "</span>";
          }
          html += "</td>";
        }
        html += "</tr>";
    ...
    hoverDate: function (e) {
      ...
      if (!$(e.target).closest(".available").length === 0) return;

      var title = $(e.target).closest(".available").attr("data-title");
      var row = title.substr(1, 1);
      var col = title.substr(3, 1);
      var cal = $(e.target).closest(".available").parents(".drp-calendar");
